...
class Details extends Component {
  ...
  onFinish = (values) => {
    console.log(values)
    // let values = this.formRef.current.validateFields().then(
    //   val => { 
  ...
  getDateDetail() {
    ...
    axios
      .get(baseUrl + 'get?type=' + id)
      .then(function (res) {
        _this.formRef.current.setFieldsValue(res.data.data)
        _this.formRef.current.setFieldsValue({'samples1': res.data.data.samples[0]});
        _this.formRef.current.setFieldsValue({'samples2': res.data.data.samples[1]});
        _this.formRef.current.setFieldsValue({'samples3': res.data.data.samples[2]});
        _this.formRef.current.setFieldsValue({'samples4': res.data.data.samples[3]});
        _this.setState({
          detail: res.data.data,
          isADV: res.data.data.isAd
        })
      })
      .catch(function (error) {
        _this.setState({
